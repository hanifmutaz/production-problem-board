...
function EditableRow({ mode, venue, initial, no, onCancel, onSaved }) {
  ...
  return (
    ...
    >
      <td className="border border-slate-100 p-2 text-center text-slate-400">
      ...
      <td className="border border-slate-100 p-1.5 text-center">
        <input type="file" accept="image/*" id={`photo-${mode}-${initial?.id || "new"}`} className="hidden" onChange={handlePhoto} />
        <label
          htmlFor={`photo-${mode}-${initial?.id || "new"}`}
          className="mx-auto flex h-10 w-10 cursor-pointer items-center justify-center overflow-hidden rounded-lg border border-dashed border-slate-300 text-slate-400 hover:border-blue-400 hover:text-blue-500"
          title="Upload foto"
        >
          {photoPreview ? <img src={photoPreview} className="h-full w-full object-cover" /> : <Camera size={14} />}
        </label>
      </td>
      <td className="border border-slate-100 p-1.5">
      ...
      <td className="border border-slate-100 p-1.5 no-print">
...
export default function ProblemTable({ rows, venue, onDelete, onPhotoClick, isAdding, onCancelAdd, onSaved }) {
          ...
          <AnimatePresence initial={false}>
            ...
            {items.length === 0 && !isAdding ? (
              <motion.tr key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <td colSpan={COL_COUNT} className="p-6 text-center text-slate-400">Belum ada data</td>
              </motion.tr>
            ) : (
              items.map((item) => {
                if (item.type === "month") {
                  return (
                    <motion.tr key={item.key} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      <td colSpan={COL_COUNT} className="border border-slate-200 bg-navy px-4 py-2 text-left text-[13px] font-bold text-white">
                        {item.label}
                      </td>
                    </motion.tr>
                  );
                ...
                if (item.type === "week") {
                  return (
                    <motion.tr key={item.key} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      <td colSpan={COL_COUNT} className="border border-slate-200 bg-slate-100 px-4 py-1.5 text-left text-xs font-semibold text-slate-500">
                        {item.label}
                      </td>
                    </motion.tr>
                  );
                  ...
                  >
                    ...
                    <td className="border border-slate-100 p-3">
                      {p.photo ? (
                        ...
                        >
                          <motion.img variants={{ hover: { scale: 1.15 } }} src={resolvePhotoUrl(p.photo)} className="h-full w-full object-cover" />
